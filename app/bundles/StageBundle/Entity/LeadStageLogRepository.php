...
class LeadStageLogRepository extends CommonRepository
{
    ...
    {
        // Handle composite primary key constraints by using a different approach
        // First, get all records that need to be updated
        ...
        if (!empty($records)) {
            ...
            foreach ($records as $record) {
                // Check if a record with the same lead_id and new stage_id already exists
                ...
                if ($existingRecord) {
                    // If a conflict exists, delete the old record instead of updating
                    $this->_em->getConnection()->createQueryBuilder()
                        ->delete(MAUTIC_TABLE_PREFIX.'stage_lead_action_log')
                    ...
                    // No conflict, so we can safely update
                    $this->_em->getConnection()->createQueryBuilder()
